...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const response = await fetch(
    `${getApiBaseUrl()}/rentals?organizationId=${encodeURIComponent(organizationId)}`,
    {
      headers: {
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const payload = (await request.json()) as CreateRentalPayload;
  const response = await fetch(`${getApiBaseUrl()}/rentals`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${accessToken}`,
